...
async def get_lighter_balance(env: dict) -> Tuple[float, float]:
    ...
    try:
        ...
        raise BalanceFetchError("Timed out while fetching Lighter balance")
    except Exception as e:
...
async def get_available_capital_and_max_position(env: dict, config: BotConfig) -> Dict:
    ...
    try:
        ...
        raise BalanceFetchError(f"EdgeX balance unavailable: {e}") from e
    ...
    try:
        ...
        raise BalanceFetchError(f"Lighter balance unavailable: {e}") from e
