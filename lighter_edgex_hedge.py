...
from utils import (
    ...
    utc_now, utc_now_iso, to_iso_z, from_iso_z,
    compute_base_size_from_quote, get_avg_mid, _calculate_apr
)
from bot_state import (
    BotState, BotConfig, StateManager, DEFAULT_SYMBOLS,
    display_funding_table, display_cycle_summary, display_status
)
...
# that explained the failure. Rotation keeps that bounded (10MB x 3 = 30MB max).
from logging.handlers import RotatingFileHandler

file_handler = RotatingFileHandler(
    'logs/auto_rotation_bot.log', mode='a', maxBytes=10 * 1024 * 1024, backupCount=3
)
...
# Flushing is intentionally left at the stdlib default.
#
# This block previously did:
#     for handler in logging.getLogger().handlers:
#         handler.flush = lambda: None   # "Force flush on every log"
#
# which did the exact OPPOSITE of its comment - it replaced flush with a no-op. Buffered
# records were then lost whenever the process died, which is precisely the moment the log
# matters (crash with a position open, followed by a container restart).
#
# No replacement is needed: logging.StreamHandler.emit() already calls self.flush() after
# every record, and RotatingFileHandler inherits that. The default was correct all along.
...
# Note: DEFAULT_SYMBOLS and load_env() are imported from bot_state.py and utils.py
...
# Note: BotState, BotConfig, StateManager imported from bot_state.py
# Note: _round_to_tick, _ceil_to_tick, _floor_to_tick, etc. imported from utils.py


# ==================== Exchange Helper Functions (continued) ====================

# get_avg_mid fallback logic (for edge cases not in utils.py)
def _get_avg_mid_extended(
    lighter_bid: Optional[float],
    lighter_ask: Optional[float],
    edgex_bid: Optional[float],
    edgex_ask: Optional[float],
) -> float:
    """Extended average mid price with extra fallbacks (wraps utils.get_avg_mid)."""
    # Try the main function first
    try:
        return get_avg_mid(lighter_bid, lighter_ask, edgex_bid, edgex_ask)
    except RuntimeError:
        pass

    # Extra fallbacks for edge cases
...
# Note: _calculate_apr, utc_now, utc_now_iso, to_iso_z, from_iso_z imported from utils.py
...
async def fetch_symbol_spread(symbol: str, quote: str, env: dict) -> Optional[float]:
    ...
    edgex_mid, lighter_mid = await asyncio.gather(fetch_edgex_mid(), fetch_lighter_mid())  # noqa: E501
...
async def fetch_symbol_funding(symbol: str, quote: str, env: dict, check_volume: bool = True, min_volume_usd: float = 250_000_000, max_spread_pct: float = 0.15) -> dict:
    ...
    # Lighter settles HOURLY -> 24 periods/day, not 3. See the note in utils.py
    # _calculate_apr; resolved empirically against Hyperliquid on the cross-venue
    # /api/v1/funding-rates endpoint (median ratio 0.9600 over 98 symbols).
    # Using 3 understated every Lighter APR by exactly 8x.
...
async def open_delta_neutral_position(
    ...
    signer = lighter.SignerClient(
        ...
        api_private_keys={int(env["API_KEY_INDEX"]): env["API_KEY_PRIVATE_KEY"]}
    )
    ...
    # Configure leverage.
    #
    # The return value used to be discarded. configure_leverage reports per-venue
    # success, and ignoring it meant the bot would happily open a "delta-neutral"
    # pair with mismatched leverage -- e.g. EdgeX still at 10x while Lighter sat at
    # 1x. The legs are then equal in size but wildly unequal in margin consumption,
    # so a move that is neutral overall can still liquidate the levered venue.
    # Refuse to trade rather than open a hedge whose risk profile is unknown.
    edgex_lev_ok, lighter_lev_ok = await configure_leverage(
        leverage, env, edgex, signer, e_contract_id, l_market_id, verify=True
    )
    if not (edgex_lev_ok and lighter_lev_ok):
        failed = []
        if not edgex_lev_ok:
            failed.append("EdgeX")
        if not lighter_lev_ok:
            failed.append("Lighter")
        await signer.close()
        await api_client.close()
        await edgex.close()
        raise RuntimeError(
            f"Refusing to open: leverage could not be set to {leverage}x on "
            f"{', '.join(failed)}. Mismatched leverage across legs is a "
            f"liquidation risk even when the position is delta-neutral."
        )
    ...
    # Normalise through the canonical classifier rather than testing
    # `isinstance(res, Exception)`.
    #
    # That test missed two whole failure conventions used right here:
    #   - lighter_place_aggressive_order returns None on error (lighter_client)
    #   - edgex_client.place_aggressive_order returns the raw response dict even
    #     when code != "SUCCESS"
    # Either one sailed through as a success, so the bot printed "Both orders
    # placed successfully" and then held a fully naked leg for the configured
    # hold_duration_hours -- 144h by default in this bot.
    leg_results = [
        classify_submission(leg_names[idx], res, intent_qty=float(size_base),
                            symbol=symbol, side="open")
    ...
    if rejected:
        ...
        # Anything not definitively rejected may be live and must be rolled back.
        successful = [leg_names[i] for i, r in enumerate(leg_results)
                      if r.status is not LegStatus.REJECTED]
        if successful:
            print("\n⚠️  CRITICAL: naked leg risk!")
            print(f"   Possibly live on: {', '.join(successful)}")
            print("🔄 Automatically rolling back leg(s) that may be open...")

            # Close any successful positions to prevent orphaned exposure
            rollback_tasks = []
            for idx, r in enumerate(leg_results):
                if r.status is not LegStatus.REJECTED:
            ...
            #
            # This used to gather() and then print "Rollback completed" whenever no
            # exception escaped -- but lighter_close_position returns False on error
            # and edgex close returns a non-SUCCESS dict, so a rollback that closed
            # NOTHING still reported success. That is the worst possible place for a
            # false positive: it is the last line of defence before a naked leg is
            # left unattended.
            if rollback_tasks:
                rb_results = await asyncio.gather(*rollback_tasks, return_exceptions=True)
                rb_classified = [
                    classify_submission(f"rollback-{i}", r, intent_qty=float(size_base),
                                        symbol=symbol, side="close")
                    for i, r in enumerate(rb_results)
                ]
                rb_failed = [r for r in rb_classified if r.status is LegStatus.REJECTED]
                if rb_failed:
                    for r in rb_failed:
                        print(f"⚠️  ROLLBACK FAILED on {r.venue}: {r.error}")
                    print("   A leg may still be OPEN and unhedged. Halting.")
                    write_halt(
                        "rollback failed after partial open",
                        symbol=symbol, venue="/".join(r.venue for r in rb_failed),
                        residual_qty=float(size_base),
                        path=os.path.join(os.path.dirname(os.path.abspath(
                            os.getenv("BOT_STATE_FILE", "logs/bot_state.json"))) or ".",
                            "halt.json"),
                    )
                else:
                    print("✓ Rollback orders accepted - verify both venues are flat")
    ...
    print("✓ Both orders accepted (fills verified below)")
...
# Note: StateManager class imported from bot_state.py
...
async def build_recovered_position_state(env: dict, config: BotConfig, symbol: str,
                                         ...
                                         edgex_contract_id: str, lighter_market_id: int,
                                         prior_opened_at: Optional[str] = None) -> dict:
    """Construct a full current_position payload from live exchange balances.

    `prior_opened_at` (ISO-Z) preserves the ORIGINAL open time across a recovery.
    Without it every recovery stamped opened_at = now, restarting the hold clock —
    so a position could never reach its target_close_at if the process restarted
    more often than hold_duration_hours. That is not hypothetical here: this bot's
    docker-compose runs `timeout 14400` (4h) against a default 144h hold, so the
    clock was reset roughly 36 times before the position was ever due to close.
    """
    ...
    # Preserve the original open time when we have one; only fall back to "now"
    # for a position we are genuinely seeing for the first time.
    opened_at = None
    if prior_opened_at:
        try:
            opened_at = from_iso_z(prior_opened_at)
            logger.info(
                "Preserving original opened_at=%s across recovery (hold clock NOT reset)",
                prior_opened_at,
            )
        except (ValueError, TypeError) as e:
            logger.warning("Could not parse prior opened_at=%r (%s); using now.",
                           prior_opened_at, e)
    if opened_at is None:
        opened_at = utc_now()
        logger.warning(
            "No prior opened_at available - stamping now. The hold clock starts "
            "from this moment, which may extend the intended hold."
        )
...
# Note: display_funding_table, display_cycle_summary, display_status imported from bot_state.py
...
async def recover_state(state_mgr: StateManager, env: dict) -> bool:
    ...
    if state == BotState.IDLE:
        ...
        try:
                ...
                if len(positions_found) == 1:
                    ...
                    if abs(abs(edgex_size) - abs(lighter_size)) < 0.1 and (edgex_size * lighter_size) < 0:
                        ...
                        position_state = await build_recovered_position_state(
                            ...
                            prior_opened_at=(state_mgr.state.get('current_position') or {}).get('opened_at'),
                        )
            ...
            logger.error(f"{Colors.RED}Could not verify positions due to error: {e}{Colors.RESET}")
            logger.error("Cannot safely proceed without verifying positions.")
            return False  # Do not allow bot to start if scan fails
...
class RotationBot:
    ...
    def __init__(self, state_file: str = "logs/bot_state.json", config_file: str = "bot_config.json"):
...
def main():
    ...
    # Must match docker-compose's BOT_STATE_FILE=/app/logs/bot_state.json. A bare
    # "bot_state.json" default meant host runs read a stale root file the container
    # never writes.
    default_state_file = os.getenv("BOT_STATE_FILE", "logs/bot_state.json")
