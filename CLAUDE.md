...
- **Funding rate caching**: Time-based cache system to avoid redundant API calls
  - `FUNDING_CACHE`: Module-level dictionary storing `{(symbol, quote, exchange): (rate, timestamp)}`
  - `FUNDING_CACHE_TTL_SECONDS`: Configurable TTL (default: 300 seconds / 5 minutes)
  - `get_cached_funding()`: Checks cache and returns rate if still valid
  - `set_cached_funding()`: Stores rate with current timestamp after successful fetch
  - `clear_funding_cache()`: Clears all cached entries (useful for testing)
  - Cache checks happen before API calls in both `fetch_edgex_rate()` and `fetch_lighter_rate()`
  - Prevents duplicate fetches when bot restarts or analyzes multiple times within short period
  - Cache hits logged at DEBUG level for visibility: "Using cached funding for BTC/USD on EdgeX (age: 45.2s / 300s)"
  - Cache misses logged at DEBUG level: "Cache expired for BTC/USD on EdgeX (age: 310.5s > 300s)"
